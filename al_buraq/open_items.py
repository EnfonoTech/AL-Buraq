...
Used by the Script Reports in al_buraq/al_buraq/report/:
    ...
    Customer Open Items Summary
    Supplier Open Items Summary
"""
...
from al_buraq.query import fetch_in
...
def cutover_date():
	...
	sf_trading (Steel Force) floors this against a Single DocType field recording where its
	ePromise migration left phantom open items. al_buraq has no equivalent migration history,
	so there is nothing to floor against here -- every caller already treats a falsy return as
	"no floor", which is the correct default for a site that never migrated.
	"""
	return None
